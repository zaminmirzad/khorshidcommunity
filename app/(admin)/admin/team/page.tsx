  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        ...
        <button onClick={openCreate} className="flex items-center gap-2 bg-accent hover:bg-accent-hover text-brand-950 font-semibold px-5 py-2.5 rounded-xl text-sm transition-all shadow-[0_4px_12px_rgba(251,191,36,0.25)] shrink-0">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-accent/30 shadow-sm p-6">
          ...
          <form onSubmit={save} className="space-y-4">
            ...
            <div className="flex justify-end gap-2">
              <button type="button" onClick={() => setShowForm(false)} className="px-4 py-2 rounded-xl text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">Cancel</button>
              <button type="submit" disabled={saving} className="px-5 py-2 rounded-xl text-sm font-semibold bg-accent hover:bg-accent-hover disabled:opacity-60 text-brand-950 transition-all">{saving ? 'Saving…' : editing ? 'Save Changes' : 'Add Member'}</button>
            </div>
          </form>
        </div>
      )}
...
function MemberCard({ member, onEdit, onDelete, onToggle }: { member: TeamMember; onEdit: (m: TeamMember) => void; onDelete: (m: TeamMember) => void; onToggle: (m: TeamMember) => void }) {
  return (
    <div className="flex items-center gap-4 bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm px-5 py-4">
      ...
      <div className="flex items-center gap-1.5 shrink-0">
        <button onClick={() => onToggle(member)} className="text-xs font-medium px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">{member.active ? 'Hide' : 'Show'}</button>
        <button onClick={() => onEdit(member)} className="text-xs font-medium px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">Edit</button>
        <button onClick={() => onDelete(member)} className="text-xs font-medium px-3 py-1.5 rounded-lg bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-950/50 transition-colors">Delete</button>
      </div>
    </div>
  );
}
