...
export async function GET() {
  ...
  const { data } = await admin.from('programs').select('*').order('sort_order').order('created_at');
...
export async function POST(request: Request) {
  ...
  if (!body.title_en) return NextResponse.json({ error: 'English title is required.' }, { status: 400 });
  const admin = createAdminClient();
