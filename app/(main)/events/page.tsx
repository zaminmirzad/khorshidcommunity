  ...
  return (
    <>
      ...
      <PageHero
        ...
        badge="Come Together"
        title={<>Events & <em className="italic text-accent-muted">Programs</em></>}
        ...
        <div className="bg-surface border-b border-gray-100 py-10">
          <div className="container mx-auto px-6 text-center">
            <span className="inline-flex items-center gap-3 text-accent-dark font-semibold uppercase text-[11px] tracking-[0.2em]">
              <span className="w-10 h-px bg-accent" />Upcoming Gatherings<span className="w-10 h-px bg-accent" />
            </span>
            <p className="text-gray-500 mt-3 text-sm max-w-lg mx-auto">Reserve your spot — all community events are open to members and guests alike.</p>
          </div>
        </div>
        ...
        <div className="container mx-auto px-6 py-20">
            ...
            {EVENTS.map((event, i) => (
              <article key={event.id} className="group bg-surface rounded-2xl overflow-hidden border border-gray-100 hover:border-accent-light hover:shadow-2xl transition-all duration-500 flex flex-col md:flex-row">
                <div className="md:w-2/5 h-60 md:h-auto relative shrink-0 overflow-hidden">
                  ...
                  <span className="absolute top-4 left-4 bg-accent text-brand-950 px-3 py-1 rounded-full font-semibold text-xs tracking-wide">{event.type}</span>
                </div>
          ...
          <div className="mt-20 max-w-3xl mx-auto">
            <div className="bg-gradient-to-br from-brand-900 via-brand-950 to-brand-950 rounded-2xl p-10 text-center text-white relative overflow-hidden">
              ...
              <div className="absolute -bottom-10 right-0 w-48 h-48 rounded-full bg-brand-400/12 blur-[60px] pointer-events-none" />
              <div className="relative z-10">
                <span className="inline-flex items-center gap-3 text-accent/80 font-semibold uppercase text-[11px] tracking-[0.2em] mb-5">
                  <span className="w-8 h-px bg-accent/50" />Have an Idea?<span className="w-8 h-px bg-accent/50" />
                </span>
                ...
                <p className="text-brand-300 text-sm mb-7 max-w-md mx-auto">We welcome ideas from our community. Reach out to our events team and let's build something great together.</p>
